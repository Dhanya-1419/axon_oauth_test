...
export async function GET(req) {
  ...
  if (!code) {
    await logActivity("docusign", "ERROR", "Missing code from provider");
    return NextResponse.redirect(`${baseUrl}?oauth_error=missing_code`);
  }

  const { clientId, clientSecret, redirectUri } = await getOAuthConfig("docusign", new URLSearchParams(), req);
  ...
  try {
    // DocuSign requires Basic auth for token exchange
    ...
    if (!tokenRes.ok) {
      throw new Error(tokenData.error_description || tokenData.error || "DocuSign token exchange failed");
    }

    const { setToken } = await import("../../tokens/route.js");
    await setToken("docusign", {
