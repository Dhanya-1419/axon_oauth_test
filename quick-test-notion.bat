@echo off
echo === Notion Connection Quick Test ===
echo.

echo 1. Testing OAuth start endpoint...
curl.exe -s -o nul -w "%%{http_code}" http://localhost:3000/api/oauth/start/notion
echo.

echo 2. Checking environment setup...
curl.exe -s http://localhost:3000/api/debug/notion
echo.
...
echo http://localhost:3000/api/oauth/start/notion
echo.
pause
